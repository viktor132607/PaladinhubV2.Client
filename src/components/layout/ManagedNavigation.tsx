...
function Menu({ item, children }: { item: NavigationEntry; children: NavigationEntry[] }) {
  ...
  return <li className="nav-item dropdown" onKeyDown={event => { if (event.key === "Escape") setOpen(false); }}>
    <div className="d-flex align-items-center"><NavAnchor item={item} className="nav-link" />
      {children.length ? <button type="button" className="nav-link border-0 bg-transparent" style={{ minWidth: 44, minHeight: 44 }} aria-label={t("nav.submenu", "Toggle {name} submenu").replace("{name}", item.name)} aria-expanded={open} aria-controls={`managed-menu-${item.id}`} onClick={() => setOpen(v => !v)}>▾</button> : null}
    </div>
