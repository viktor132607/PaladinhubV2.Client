...
export default function Footer(){
 ...
 const text=(x:Entry)=>t(`footer.${x.id}.text`,x.text.replace("Made with 💛 for WoW Paladins",t("footer.credit", "Made with 💛 for WoW Paladins"))).replaceAll("{year}",String(new Date().getFullYear()));
 ...
 return <footer className="border-top footer text-muted text-center mt-5 py-3 ph-v1-footer"><div className="container">
  {entries===null?<>© {new Date().getFullYear()} - PaladinHub | {t("footer.credit", "Made with 💛 for WoW Paladins")}</>:<div className="d-flex flex-wrap justify-content-center gap-4">{entries.filter(x=>x.kind==="section").map(section=><section key={section.id} style={{flex:"1 1 200px",minWidth:0,maxWidth:"100%"}}>{section.text&&<h2 className="h6">{text(section)}</h2>}<ul className="list-unstyled mb-0">{entries.filter(x=>x.parentId===section.id).map(x=><li key={x.id}>{render(x)}</li>)}</ul></section>)}</div>}
 </div></footer>;
}
