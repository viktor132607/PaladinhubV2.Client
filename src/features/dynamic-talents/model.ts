...
export function validateTree(tree: Tree): string[] {
  ...
  for (const node of tree.nodes) {
    ...
    if (cells.has(cell))
      errors.push("Two talents cannot occupy the same cell.");
    cells.add(cell);
    if (
      ...
      errors.push(
        "A talent is outside the grid. Move it before shrinking the grid.",
      );
    if (
  ...
  function visit(id: string): boolean {
    ...
    visiting.add(id);
    for (const parent of node.requires) if (!visit(parent)) return false;
    visiting.delete(id);
    done.add(id);
    return true;
  }
  if (tree.nodes.some((n) => !visit(n.id)))
    errors.push(
      "Connections must reference existing talents and cannot form a cycle.",
    );
  return errors;
}
...
export function parseLayout(json: string): Block[] {
  ...
  if (
    !Array.isArray(value) ||
    value.some((b) => !b || typeof b !== "object" || typeof b.type !== "string")
  )
    throw new Error("Invalid page layout.");
  return value;
}
export function treeBlocks(json: string): Tree[] {
  return parseLayout(json).filter(
    (b) => b.type === "talenttree.dynamic",
  ) as Tree[];
}
export function replaceTree(json: string, tree: Tree): string {
  ...
  const index = blocks.findIndex(
    (b) => b.type === tree.type && b.id === tree.id,
  );
