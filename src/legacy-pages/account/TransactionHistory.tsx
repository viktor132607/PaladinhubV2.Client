...
const regions = ["Europe", "Americas", "Asia", "All"] as const;
...
function normalizeRegion(value: unknown): Region {
  ...
  return regions.includes(region as Region) ? (region as Region) : "Europe";
}
...
function normalizeTransactionItem(value: unknown): TransactionItem | null {
  const source = asRecord(value);

  const dateUtc = String(property(source, "dateUtc", "DateUtc") ?? "").trim();

  const purchase = String(
    property(source, "purchase", "Purchase") ?? "",
  ).trim();

  const total = String(property(source, "total", "Total") ?? "").trim();

  const status = String(property(source, "status", "Status") ?? "").trim();

  if (!dateUtc && !purchase && !total && !status) {
    return null;
  }
...
function normalizeHistory(payload: unknown): TransactionHistoryResponse {
  const source = asRecord(payload);

  const rawItems = property(source, "items", "Items");
  ...
  const totalPages = normalizePositiveInteger(
    property(source, "totalPages", "TotalPages"),
    1,
  );

  const page = Math.min(
    normalizePositiveInteger(property(source, "page", "Page"), 1),
    totalPages,
  );
  ...
  return {
    ...
    page,
    totalPages,
    region: normalizeRegion(property(source, "region", "Region")),
  };
}
...
export default function TransactionHistory() {
  ...
  const requestedPage = normalizePositiveInteger(searchParams.get("page"), 1);

  const requestedRegion = normalizeRegion(searchParams.get("region"));
  ...
  const navigate = (page: number, region: Region = requestedRegion): void => {
    setSearchParams({
  ...
  return (
    <AccountLayout active="TransactionHistory">
      ...
      <div className={s.history}>
        <div className="mb-4 flex flex-wrap items-end gap-3 border-b border-[#313a45]">
          ...
          <span className="px-4 py-3 text-[#68717d]">Gift Claims</span>

          <span className="px-4 py-3 text-[#68717d]">Virtual Wallet</span>
          ...
          <label className="mb-2 ml-auto flex items-center gap-2 text-sm text-[#a8b0bd]">
            ...
            <select
              ...
              disabled={loading}
              onChange={(event) => {
                navigate(1, normalizeRegion(event.target.value));
              }}
                  ...
                  ? history.items.map((item, index) => (
                      ...
                      >
                        <td className="px-4 py-4">
                          {formatDate(item.dateUtc)}
                        </td>

                        <td className="px-4 py-4">{item.purchase || "—"}</td>
