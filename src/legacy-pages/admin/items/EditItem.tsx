...
export default function EditItem() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const item = await readApiJson<ItemDto>(response);
        ...
        setForm({
          ...
          url: item.url ?? "",
          itemLevel: item.itemLevel == null ? "" : String(item.itemLevel),
          requiredLevel:
            item.requiredLevel == null ? "" : String(item.requiredLevel),
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!hasValidId || !itemLoaded || saving) {
      return;
    }
      ...
      <main className="px-4 py-8 text-slate-100">
        <section className="mx-auto max-w-3xl rounded-xl border border-red-500/40 bg-slate-900 p-6 shadow-xl">
          ...
          <Link
            ...
            className="mt-5 inline-flex rounded-md bg-slate-700 px-5 py-2.5 font-medium text-white hover:bg-slate-600"
          >
            Back
          </Link>
        </section>
  ...
  return (
    <main className="px-4 py-8 text-slate-100">
      <section className="mx-auto max-w-3xl rounded-xl border border-slate-700 bg-slate-900 p-6 shadow-xl">
        <h2 className="text-2xl font-semibold">Edit Item</h2>
        <p className="mt-1 text-sm text-slate-400">Item ID: {itemId}</p>
