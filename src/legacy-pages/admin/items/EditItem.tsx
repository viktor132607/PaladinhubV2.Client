...
export default function EditItem() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError("");
      try {
        const response = await fetchBackend(`${itemsApiEndpoint}/${itemId}/edit`, {
        ...
        setForm({
          ...
          requiredLevel: item.requiredLevel == null ? "" : String(item.requiredLevel),
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!hasValidId || !itemLoaded || saving) return;
  ...
  if (!itemLoaded) {
    return (
      <>
        <h2>Edit Item</h2>
        <div className="text-danger mb-3" role="alert">{error || "The item could not be loaded."}</div>
