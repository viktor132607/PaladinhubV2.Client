...
const endpoint = "/Admin/api/patches";
...
export default function Patches() {
  ...
  const canCreate = hasPermission(adminPermissions.patches.create);
  const canUpdate = hasPermission(adminPermissions.patches.update);
  const canDelete = hasPermission(adminPermissions.patches.delete);
  const canRestore = hasPermission(adminPermissions.patches.restore);
  ...
  const mutate = async (path: string, method: string, body?: unknown) => {
    ...
    try {
      ...
      setNotice(method === "DELETE" ? "Patch moved to Deleted. Its history is preserved." : "Patch saved.");
    } catch (error) { setError(error instanceof Error ? error.message : "Could not save patch."); }
  ...
  return <section>
    ...
    <div className="d-flex flex-wrap gap-2 mb-3">
      {canCreate ? <button type="button" className="btn btn-primary" disabled={busy} onClick={() => choose(null)}>New patch</button> : null}
    ...
    <div className="row g-3">
      <div className="col-12 col-xl-7">
        <div className="d-flex flex-wrap gap-2 mb-3">
          <input type="search" className="form-control" style={{ flex: "1 1 180px", minWidth: 0 }} aria-label="Search patches" placeholder="Search patches…" value={search} onChange={event => setSearch(event.target.value)} />
          <select className="form-select" style={{ flex: "1 1 140px", minWidth: 0 }} aria-label="Patch status" value={status} onChange={event => setStatus(event.target.value)}>
        ...
        {loading ? <p>Loading patches…</p> : <>
          <p>{visible.length} patches</p>
          <table className="table table-dark table-striped admin-record-table">
            <thead><tr><th>Patch</th><th>Status</th><th>Records</th><th>Actions</th></tr></thead>
            <tbody>{visible.map(category => <tr key={category.id}>
              <td data-label="Patch" style={{ overflowWrap: "anywhere" }}>{category.name}</td>
          ...
          {!visible.length ? <p>No patches match this filter.</p> : null}
        </>}
      </div>
      <div className="col-12 col-xl-5" style={{ minWidth: 0 }}>
        <h3>{selected ? `${selected.isDeleted ? "Deleted" : canUpdate ? "Edit" : "View"} patch #${selected.id}` : canCreate ? "New patch" : "Patch details"}</h3>
        <form onSubmit={submit}>
          <fieldset disabled={busy || selected?.isDeleted || !canEditDraft}>
            ...
            <div className="d-flex flex-wrap gap-2">
              {canEditDraft ? <button type="submit" className="btn btn-success">{busy ? "Saving…" : selected ? "Save patch" : "Create patch"}</button> : null}
              ...
              }}>Delete patch</button> : null}
            </div>
          </fieldset>
        </form>
        {selected && !selected.isDeleted && (selected.usageCount > 0) ? <p className="mt-2">Remove the patch from assigned records before deleting, or archive it.</p> : null}
        {selected ? <section className="mt-4">
          ...
          {history.map(revision => {
            ...
            return <details className="border rounded p-2 mb-2" key={revision.id}>
              ...
              <p>{snapshot.Name} · Order: {snapshot.SortOrder} · {snapshot.IsArchived ? "Archived" : "Active"}</p>
              ...
              {!snapshot.IsDeleted && canRestore ? <button type="button" className="btn btn-outline-primary" disabled={busy || revision.version === selected.version} onClick={() => {
                if (window.confirm(`Restore patch to revision ${revision.version}?`)) void mutate(`${endpoint}/${selected.id}/restore`, "POST", { revisionId: revision.id, version: selected.version });
