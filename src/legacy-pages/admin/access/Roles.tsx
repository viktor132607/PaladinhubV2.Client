      ...
      <div className="row g-3">
        ...
        <div className="col-12 col-xl-9">{!selected ? <div className="card card-body text-muted">Select a role.</div> : <div className="d-grid gap-3">
          <section className="card card-body">
            ...
            <div className="row g-3 align-items-end"><div className="col-12 col-md-7"><label className="form-label" htmlFor="role-name">Name</label><input id="role-name" className="form-control" value={draftName} onChange={(event) => setDraftName(event.target.value)} disabled={!canUpdate || selected.isDeleted || selected.isSystem || busy} /></div><div className="col-12 col-md-5"><div className="form-check"><input id="role-disabled" className="form-check-input" type="checkbox" checked={draftDisabled} onChange={(event) => setDraftDisabled(event.target.checked)} disabled={!canUpdate || selected.isDeleted || selected.isSystem || busy} /><label className="form-check-label" htmlFor="role-disabled">Disable this role</label></div></div></div>
            <div className="d-flex flex-wrap gap-2 mt-3">
              {canUpdate ? <button className="btn btn-primary" type="button" disabled={busy || selected.isDeleted || selected.isSystem || !draftName.trim()} onClick={() => void run(async () => { const updated = await accessControlApi.updateRole(selected.id, draftName.trim(), draftDisabled, selected.version); await replaceSelected(updated, "Role settings saved."); }, "Role settings saved.")}>Save settings</button> : null}
              {canDelete ? <button className="btn btn-outline-danger" type="button" disabled={busy || selected.isDeleted || selected.isSystem || selected.userCount > 0} onClick={() => { if (!window.confirm(`Delete role “${selected.name}”?`)) return; void run(async () => { await accessControlApi.deleteRole(selected.id, selected.version); setSelected(null); setSelectedId(""); await loadRoles(); }, "Role deleted."); }}>Delete role</button> : null}
            </div>
          </section>

          {canReadPermissions ? <section className="card card-body">
            <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3"><div><h2 className="h5 mb-1">Permissions</h2><div className="text-muted small">{draftPermissions.length} effective catalog grant(s) selected</div></div>{canUpdatePermissions ? <button className="btn btn-primary" type="button" disabled={busy || selected.isDeleted || selected.isSystem} onClick={() => void run(async () => { const updated = await accessControlApi.replacePermissions(selected.id, draftPermissions, selected.version); await replaceSelected(updated, "Permissions saved."); }, "Permissions saved.")}>Save permissions</button> : null}</div>
            <div className="row g-3">{groupedCatalog.map((group) => <div className="col-12 col-md-6 col-xxl-4" key={group.resource}><fieldset className="border rounded p-3 h-100"><legend className="float-none w-auto px-1 fs-6 text-capitalize">{group.resource.replaceAll("_", " ")}</legend>{group.permissions.map((permission) => <div className="form-check" key={permission.id}><input className="form-check-input" id={`permission-${permission.id}`} type="checkbox" checked={draftPermissions.includes(permission.id)} disabled={!canUpdatePermissions || selected.isDeleted || selected.isSystem || busy} onChange={() => togglePermission(permission.id)} /><label className="form-check-label" htmlFor={`permission-${permission.id}`} title={permission.description}>{permission.operation}</label></div>)}</fieldset></div>)}</div>
