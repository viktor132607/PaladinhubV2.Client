...
const endpoint = "/Admin/api/classes";
...
export default function Classes() {
  ...
  const canCreate = hasPermission(adminPermissions.classes.create);
  const canUpdate = hasPermission(adminPermissions.classes.update);
  const canDelete = hasPermission(adminPermissions.classes.delete);
  const canRestore = hasPermission(adminPermissions.classes.restore);
  ...
  const mutate = async (path: string, method: string, body?: unknown) => {
    ...
    try {
      ...
      setNotice(method === "DELETE" ? "Class / specialization moved to Deleted. Its history is preserved." : "Class / specialization saved.");
    } catch (error) { setError(error instanceof Error ? error.message : "Could not save class or specialization."); }
  ...
  return <section>
    ...
    <div className="d-flex flex-wrap gap-2 mb-3">
      {canCreate ? <button type="button" className="btn btn-primary" disabled={busy} onClick={() => choose(null)}>New class / specialization</button> : null}
    ...
    <div className="row g-3">
      <div className="col-12 col-xl-7">
        <div className="d-flex flex-wrap gap-2 mb-3">
          <input type="search" className="form-control" style={{ flex: "1 1 180px", minWidth: 0 }} aria-label="Search classes and specializations" placeholder="Search classes and specializations…" value={search} onChange={event => setSearch(event.target.value)} />
          <select className="form-select" style={{ flex: "1 1 140px", minWidth: 0 }} aria-label="Class / specialization status" value={status} onChange={event => setStatus(event.target.value)}>
        ...
        {loading ? <p>Loading classes and specializations…</p> : <>
          <p>{visible.length} classes and specializations</p>
          <table className="table table-dark table-striped admin-record-table">
            <thead><tr><th>Class / specialization</th><th>Status</th><th>Records</th><th>Specializations</th><th>Actions</th></tr></thead>
            <tbody>{visible.map(category => <tr key={category.id}>
              <td data-label="Class / specialization" style={{ overflowWrap: "anywhere" }}>{categoryPath(category.id, categories)}</td>
              <td data-label="Status">{category.isDeleted ? "Deleted" : category.isArchived ? "Archived" : "Active"}</td>
              <td data-label="Records">{category.usageCount}</td><td data-label="Specializations">{category.childCount}</td>
          ...
          {!visible.length ? <p>No classes or specializations match this filter.</p> : null}
        </>}
      </div>
      <div className="col-12 col-xl-5" style={{ minWidth: 0 }}>
        <h3>{selected ? `${selected.isDeleted ? "Deleted" : canUpdate ? "Edit" : "View"} entry #${selected.id}` : canCreate ? "New class / specialization" : "Class / specialization details"}</h3>
        <form onSubmit={submit}>
          <fieldset disabled={busy || selected?.isDeleted || !canEditDraft}>
            ...
            <label className="form-label" htmlFor="category-parent">Class</label>
            <select id="category-parent" className="form-select mb-3" value={draft.parentId} onChange={event => setDraft({ ...draft, parentId: event.target.value })}>
              <option value="">None — create a class</option>
              {categories.filter(category => !category.isDeleted && category.parentId === null && !excluded.has(category.id) && (!category.isArchived || draft.isArchived)).map(category =>
            ...
            <div className="d-flex flex-wrap gap-2">
              {canEditDraft ? <button type="submit" className="btn btn-success">{busy ? "Saving…" : selected ? "Save class / specialization" : "Create class / specialization"}</button> : null}
              ...
              }}>Delete class / specialization</button> : null}
            </div>
          </fieldset>
        </form>
        {selected && !selected.isDeleted && (selected.usageCount > 0 || selected.childCount > 0) ? <p className="mt-2">Move assigned records and specializations before deleting, or archive the entry.</p> : null}
