...
export default function Translations() {
  ...
  const canCreate = hasPermission(adminPermissions.localization.create);
  const canUpdate = hasPermission(adminPermissions.localization.update);
  const canArchive = hasPermission(adminPermissions.localization.archive);
  const canDelete = hasPermission(adminPermissions.localization.delete);
  const canRestore = hasPermission(adminPermissions.localization.restore);
  ...
  function discard() { return !dirty || !editable || window.confirm("Discard unsaved translation changes?"); }
  ...
  const entries = Object.entries(resources).filter(([entryKey,entryValue]) => `${entryKey} ${entryValue}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <main className="min-w-0 space-y-5 p-3 text-slate-100 sm:p-6">
    ...
    <fieldset disabled={busy} className="min-w-0 space-y-5">
      <div className="flex flex-wrap gap-3">
        {canCreate ? <button className={button} type="button" onClick={() => { if (discard()) load(null); }}>New language</button> : null}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label>Language code<input className={field} value={code} maxLength={35} placeholder="bg, en, de, pt-BR" disabled={!!selected || !canCreate} onChange={e => setCode(e.target.value)} /></label>
      ...
      <p className="text-sm text-slate-400">{Object.keys(resources).length} translations · {entries.length} shown{dirty && editable ? " · Unsaved changes" : ""}. Removing a key restores the English fallback after saving.</p>
      <div className="max-h-[60vh] space-y-3 overflow-y-auto">{entries.slice(currentPage * 50, (currentPage + 1) * 50).map(([entryKey,entryValue]) => <div key={entryKey} className="min-w-0 rounded border border-slate-700 p-3">
        ...
        {editable ? <button type="button" className={`${button} mt-2`} aria-label={`Remove key: ${entryKey}`} onClick={() => setResources(current => Object.fromEntries(Object.entries(current).filter(([k]) => k !== entryKey)))}>Remove key</button> : null}
      </div>)}</div>
      ...
      <div className="flex flex-wrap gap-2">
        {canEditDraft ? <button type="button" className={button} disabled={!editable || !code.trim() || !name.trim()} onClick={() => void save()}>Save language & translations</button> : null}
        {selected && !selected.isDeleted && <>{canArchive ? <button type="button" className={button} disabled={selected.code === "en"} onClick={() => void change(selected.isArchived ? "unarchive" : "archive")}>{selected.isArchived ? "Unarchive" : "Archive"}</button> : null}{canDelete ? <button type="button" className={button} disabled={selected.code === "en"} onClick={() => void change("delete")}>Delete language</button> : null}</>}
      </div>
      {selected && <section className="space-y-2"><h2 className="text-xl">Translation history</h2>{history.map(r => <div key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded border border-slate-700 p-3"><span className="min-w-0 break-words">v{r.version} · {r.action} · {r.actor} · {new Date(r.createdAtUtc).toLocaleString()}</span>{canRestore ? <button type="button" className={button} disabled={JSON.parse(r.snapshot).IsDeleted} onClick={() => void change("restore",r.id)}>Restore v{r.version}</button> : null}</div>)}</section>}
    </fieldset>
  </main>;
}
