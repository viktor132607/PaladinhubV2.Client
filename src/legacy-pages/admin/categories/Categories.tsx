...
export default function Categories() {
  ...
  const canCreate = hasPermission(adminPermissions.categories.create);
  const canUpdate = hasPermission(adminPermissions.categories.update);
  const canDelete = hasPermission(adminPermissions.categories.delete);
  const canRestore = hasPermission(adminPermissions.categories.restore);
  ...
  return <section>
    ...
    <div className="d-flex flex-wrap gap-2 mb-3">
      {canCreate ? <button type="button" className="btn btn-primary" disabled={busy} onClick={() => choose(null)}>New category</button> : null}
    ...
    <div className="row g-3">
      ...
      <div className="col-12 col-xl-5" style={{ minWidth: 0 }}>
        <h3>{selected ? `${selected.isDeleted ? "Deleted" : canUpdate ? "Edit" : "View"} category #${selected.id}` : canCreate ? "New category" : "Category details"}</h3>
        <form onSubmit={submit}>
          <fieldset disabled={busy || selected?.isDeleted || !canEditDraft}>
            ...
            <div className="d-flex flex-wrap gap-2">
              {canEditDraft ? <button type="submit" className="btn btn-success">{busy ? "Saving…" : selected ? "Save category" : "Create category"}</button> : null}
